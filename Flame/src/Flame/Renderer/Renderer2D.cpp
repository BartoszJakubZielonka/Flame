...
namespace flame
{
	...
	{
		//Bind textures.
		for(uint32_t it{0}; it < s_Data.TextureSlotIndex; it++)
		{
			s_Data.TextureSlots[it]->Bind(it);
		}
	...
	void Renderer2D::DrawQuad(const glm::vec3& position, const glm::vec2& size, const glm::vec4& color)
	{
		...
		s_Data.QuadVertexBufferPtr->TexIndex = textureIndex; s_Data.QuadVertexBufferPtr->TilingFactor = tilingFactor;
	...
	void Renderer2D::DrawQuad(const glm::vec3& position, const glm::vec2& size, const Ref<Texture2D>& texture, float tilingFactor, const glm::vec4& tintColor)
	{
		...
		for(int it{0}; it < s_Data.TextureSlotIndex; it++)
		{
			if(*s_Data.TextureSlots[it].get() == *texture.get())
			{
				textureIndex = static_cast<float>(it);
				break;
			}
		}
